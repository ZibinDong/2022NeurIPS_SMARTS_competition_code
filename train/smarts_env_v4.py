...
class EnvWrapper(gym.Wrapper):
        ...
        for i in range(5):
            if turn_left:
                if  (neighbor_x[i] < -1.6 and neighbor_x[i] > -4.0) and \
                    ...
                    (neighbor_h[i] < 0.05):
                        print(f'neighbor speed: {neighbor_v[i]}')
                        return False
            else:
                if  (neighbor_x[i] >  1.6 and neighbor_x[i] <  4.0) and \
                    ((neighbor_y[i] <  0.0 and neighbor_y[i] > min(-9.0, -1.5*neighbor_v[i].item())) or \
    ...
    def add_target_lane_obs(self, obs):
        for agent_id in self.agents_id:
            s_target_lane = np.zeros(3)
            if self.env.target_lane_index[agent_id] < self.env.preserved_info[agent_id].lane_index:
                s_target_lane[0] = 1.0
            elif self.env.target_lane_index[agent_id] > self.env.preserved_info[agent_id].lane_index:
                s_target_lane[2] = 1.0
            else:
                s_target_lane[1] = 1.0
    ...
    def pack_info(self, info):
        for agent_id in self.agents_id:
            info[agent_id]["humaness"] = self.env.preserved_info[agent_id].humaness
        return info
    ...
    def pack_action(self, action):
        ...
        for agent_id in self.agents_id:
            ...
            acc = 0.3 if exp_speed > speed else max(0.8, acc)
    ...
    def pack_reward(self, raw_act, done):
        ...
        for agent_id in self.agents_id:
            r_forward = self.env.preserved_info[agent_id].raw_obs.distance_travelled if \
                self.env.preserved_info["Agent_0"].raw_obs.ego_vehicle_state.speed > 0.01 else 0
            events = self.env.preserved_info[agent_id].raw_obs.events
            r_events = 0
            if events.collisions: 
                r_events  -= 50
                self.crash_times += 1
            if events.off_route:    r_events -= 30
            if events.wrong_way:    r_events -= 30
            if events.reached_goal: r_events += 100
            if events.on_shoulder:  r_events -= 30
            if events.off_road:     r_events -= 30
    
            r_time_cost = self.time_cost
            
            r_lane_change = 0
            if raw_act[agent_id] in [int(i) for i in range(6)]:
                r_lane_change -= 3
            elif raw_act[agent_id] in [10]:
                r_lane_change -= 0
                
            # if self._is_on_goal_lane_last != -1 and (self._is_on_goal_lane_last != self.env.preserved_info[agent_id].is_on_goal_lane):
            #     r_goal = (self.env.preserved_info[agent_id].is_on_goal_lane - self._is_on_goal_lane_last) * 100
            # else: r_goal = 0
            # self._is_on_goal_lane_last = self.env.preserved_info[agent_id].is_on_goal_lane
            r_goal = 1.0 if self.env.preserved_info[agent_id].is_on_goal_lane else 0.0
            if done[agent_id] and hasattr(self.env.preserved_info[agent_id].raw_obs.ego_vehicle_state.mission.goal, "position"):
                position = self.env.preserved_info[agent_id].raw_obs.ego_vehicle_state.position[:2]
                goal_position = np.array(self.env.preserved_info[agent_id].raw_obs.ego_vehicle_state.mission.goal.position[:2])
                goal_distance = self.cal_distance(position, goal_position)
                if goal_distance < 20:
                    r_goal += min(5 / goal_distance, 10)
                    
            r_over_speed = - max(0, (self.env.preserved_info[agent_id].speed - self.env.preserved_info[agent_id].speed_limit).item())
...
class SingleAgentWrapper(EnvWrapper):
    def __init__(self, env, default_agent_id = "Agent_0"):
        ...
        self.observation_space = gym.spaces.Box(-1e10, 1e10, self.observation_space_shape)
        
    def reset(self):
        return super().reset()[self.agent_id].astype(np.float32)
        
    def step(self, action):
        obs, reward, done, info = super().step({self.agent_id: action})
        info[self.agent_id]["success_rate"] = 1.0 if self.env.preserved_info[self.agent_id].np_obs["events"]["reached_goal"] else 0.0
        return obs[self.agent_id].astype(np.float32), reward[self.agent_id], done[self.agent_id], info[self.agent_id]
        

class EndlessGoalWrapper(SingleAgentWrapper):
    '''
    Need to modify multi_scenario_env.py 
    ...
    '''
    def __init__(self, env, default_agent_id = "Agent_0", max_episode_len = 500):
        super().__init__(env, default_agent_id)
    ...
    def step(self, action):
        ...
        self._step += 1
        info["crash"] = self.crash_times
        info["reached_goal"] = info["env_obs"].events.reached_goal
        if self._step == self._max_episode_len:
            done = True
        else:
            if done:
                self._total_episodes += 1
                if info["env_obs"].events.reached_goal and self.crash_times == 0: self._success_episodes += 1
                obs = super().reset()
                done = False
        info["step"] = self._step
        info["total_episodes"] = self._total_episodes
        info["success_episodes"] = self._success_episodes
        info["success_rate"] = (self._success_episodes/self._total_episodes) if self._total_episodes != 0 else 0.0
        info["success_rate"] = 1.0 if info["crash"] == 0 and self._total_episodes == 0 else info["success_rate"]
        
        return obs, reward, done, info
        
        
map_list = [[0]*3 for _ in range(6)]
for i in range(2):
    for j in range(3):
        map_list[i][j] = i*2+j
for i in [2,3,4,5]:
    for j in range(3):
        map_list[i][j] = 4+(i-2)*3+j


map_feats_simple = ["left_c", "left_t", "right_c", "right_t", "cruise", "merge", "cut_in", "overtake", "curve"]
scenario_list = []
scenario_list_id = {}
counter = 0
for map_feat in map_feats_simple:
    n = 2 if map_feat in ["left_c", "left_t", "right_c", "right_t"] else 3
    if map_feat == "curve": n = 1
    for i in range(n):
        scenario_list.append(map_feat+"_"+str(i))
        scenario_list_id[scenario_list[-1]] = counter
        counter += 1


class MultiScenarioEnv(gym.Env):
    def __init__(self, envs, average_sample = False):
        super().__init__()
        self.envs = envs
    ...
    def env(self) -> gym.Env:
        return self.envs[self.current_scenario_index]
    
    def reset(self):
        if not self.average_sample:
            self.current_scenario_index = (self.current_scenario_index + 1) % self.n_envs
            ...
            map_feat = random.choice(map_feats_simple)
            i = random.randint(0, 1 if map_feat in ["left_c", "left_t", "right_c", "right_t"] else 2)
            if map_feat == "curve": i = 0
            self.current_scenario_index = scenario_list_id[map_feat+"_"+str(i)]
            self.z_map = map_feats_simple.index(map_feat)
        return self.env.reset()
    
    def step(self, action):
        o, r, d, info = self.env.step(action)
        if self.average_sample:
            info["classifier_data"] = (self.z_map, self.env.preserved_info["Agent_0"].classifier_input)
        return o, r, d, info
...
def make(
    ...
    is_endless_goal: bool = False,
    is_multi_scenario: bool = False,
    average_sample: bool = False,
):
    if not is_multi_scenario:
        raw_env = gym.make(
            "smarts.env:multi-scenario-v0",
            scenario=scenario,
            visdom=visdom,
        )
        env0 = ObsWrapper(raw_env)
        if is_multi_agent:
            env = EnvWrapper(env0)
        elif is_endless_goal:
            env = EndlessGoalWrapper(env0, default_agent_id)
        else:
            env = SingleAgentWrapper(env0, default_agent_id)
    else:
        raw_env = [
            ObsWrapper(gym.make(
                "smarts.env:multi-scenario-v0",
        ...
        if is_multi_agent:
            envs = [EnvWrapper(env0) for env0 in raw_env]
        elif is_endless_goal:
            envs = [EndlessGoalWrapper(env0, default_agent_id) for env0 in raw_env]
        else:
            envs = [SingleAgentWrapper(env0, default_agent_id) for env0 in raw_env]
        env = MultiScenarioEnv(envs, average_sample)
    return env
